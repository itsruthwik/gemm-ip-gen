...
from generate_catapult_rtl import generate_sim_verilog, generate_synth_verilog, generate_combined_core_verilog
...
def _run_sim(rtl_src, tb_src, rtl_file, tb_file, extra_v=None, timeout=900):
    ...
    # Extract BEH_START/BEH_DONE/BEH_II and WRAP_START/WRAP_DONE
    ...
    for line in r.stdout.splitlines():
        ...
        elif line.startswith("WRAP_START"):
            wr_starts.append(int(line.split("=")[-1]))
        elif line.startswith("WRAP_DONE"):
            wr_dones.append(int(line.split("=")[-1]))
    if bh_starts and len(bh_starts) == len(bh_dones):
        bh_lat = [d - s + 1 for d, s in zip(bh_dones, bh_starts)]
    else:
        bh_lat = []
    if wr_starts and len(wr_starts) == len(wr_dones):
        wr_lat = [d - s + 1 for d, s in zip(wr_dones, wr_starts)]
    else:
        wr_lat = []
    if bh_lat or wr_lat:
        parts = []
        if bh_lat: parts.append(f"beh={bh_lat[0]}")
        if bh_iis:
            steady_ii = [x for x in bh_iis[1:] if x > 0]
            if steady_ii: parts.append(f"II={steady_ii[0]}")
        if wr_lat: parts.append(f"wrap={wr_lat[0]}")
        n = max(len(bh_lat), len(wr_lat))
        consistent = all(l == bh_lat[0] for l in bh_lat) and all(l == wr_lat[0] for l in wr_lat)
...
class TestSynthStructuralSmoke:
    ...
    def _compile_with_stub(rtl, mod, tmp_path, define_synth=False):
        ...
        r = subprocess.run(
            ["iverilog", "-g2012", "-s", mod, "-o", str(out_file), str(rtl_file), str(stub_file)],
            text=True,
            capture_output=True,
            timeout=60,
        )
