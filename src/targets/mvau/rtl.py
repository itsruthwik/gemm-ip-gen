...
def generate_two_operand_shim(shape, module_name="mvau_core", force_behavioral=True,
    ...
    in RTL. B arrives as **N-wide beats in K-row order** (row-major ``B[k][:]``), so a
    small load FSM gathers SIMD consecutive rows into one memstream word (bit
    ``(pe*SIMD+s)*WW`` = ``B[sf*SIMD+s][pe]``) and writes it at address ``sf``.

    MVP scope: single tile, NF=1 (PE=N), any SF; no N/K-tiling. The controller runs
    LOAD (config_ce held high so the stream pointer stays at 0; config_we pulsed per
    completed word) -> a 2-cycle DRAIN (let the last write commit) -> RUN (config_ce
    low, activations stream, memstream replays), then reloads for the next node."""
    ...
    if t["nf"] != 1:
        raise NotImplementedError(
            f"two-operand shim MVP supports NF=1 (PE=N) only; got NF={t['nf']} "
            f"(PE={t['pe']}, N={p['n']}). Temporal-N load logic is a later step.")
    fb = 1 if force_behavioral else 0
    N = p["n"]
    M = p["num_input_vectors"]
    SIMD, SF, WW, ACCU = t["simd"], t["sf"], t["weight_width"], t["accu_width"]
    ...
    DEPTH = SF                            # NF*SF, NF=1
    aw_ptr = max(1, (DEPTH - 1).bit_length())
    run_total = M                         # M*NF output beats per node (NF=1)
    # combinational per-column fill of the current word (wacc + this beat at lane sc)
    fill = "\n".join(
        f"        wword[{(pe) * SIMD * WW} + sc*{WW} +: {WW}] = b_dout[{pe * WW} +: {WW}];"
        for pe in range(N))
...
// Tile: MW(K)={t['mw']} MH(N)={N} PE={t['pe']} SIMD={SIMD} SF={SF} NF=1 \
...
module {module_name} (
    ...
    wire rst_n = ~ap_rst;
    localparam [1:0] LOAD = 2'd0, DRAIN = 2'd1, RUN = 2'd2;

    reg  [1:0]            state = LOAD;
    reg  [{max(1, (SIMD - 1).bit_length()) - 1 if SIMD > 1 else 0}:0]  sc = 0;    // lane within band (0..SIMD-1)
    reg  [{aw_ptr - 1}:0]  sfc = 0;   // word index (0..SF-1)
    reg  [{WB - 1}:0]  wacc = 0;      // word accumulator
    reg  [1:0]            dcnt = 0;   // drain counter
    reg  [{max(1, run_total.bit_length()) - 1}:0]  ocnt = 0;   // output beats seen this node

    // b handshake: consume a B beat only while loading
    assign b_read = ap_ce & (state == LOAD) & b_empty_n;
    wire   band_last = (sc == {SIMD - 1});

    // current word = accumulated lanes + this beat placed at lane sc
    reg  [{WB - 1}:0]  wword;
    always @* begin
        wword = wacc;
{fill}
    ...
    // memstream config-write load port
    wire        cfg_ce = ap_ce & (state == LOAD || state == DRAIN);
    wire        cfg_we = ap_ce & (state == LOAD) & b_read & band_last;
    wire [31:0] cfg_addr = sfc;   // zero-extended word address
    wire [{WB - 1}:0] cfg_d0 = wword;

    always @(posedge ap_clk) begin
        if (ap_rst) begin
            state <= LOAD; sc <= 0; sfc <= 0; wacc <= 0; dcnt <= 0; ocnt <= 0;
        end else if (ap_ce) begin
            case (state)
                LOAD: if (b_read) begin
                    if (band_last) begin
                        wacc <= 0; sc <= 0;
                        if (sfc == {SF - 1}) begin sfc <= 0; state <= DRAIN; dcnt <= 0; end
                        else sfc <= sfc + 1'b1;
                    end else begin
                        wacc <= wword; sc <= sc + 1'b1;
                    end
                end
                DRAIN: begin
                    dcnt <= dcnt + 1'b1;
                ...
                RUN: if (p_write) begin
                    if (ocnt == {run_total - 1}) begin state <= LOAD; ocnt <= 0; end
                    else ocnt <= ocnt + 1'b1;
                end
            endcase
        end
