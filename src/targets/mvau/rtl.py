...
def generate_two_operand_nt_shim(shape, module_name="mvau_core", force_behavioral=True,
                                 tile=None, plan=None, **plan_kwargs):
    """N-tiled two-operand shim: ``nt`` MVU cores, each computing the FULL-K reduction for its
    own N-column block (distinct outputs, concatenated -- no summing). Activation is broadcast
    (every tile does the full contraction); B is column-sliced per tile. Each tile keeps its
    own runtime ``memstream`` (DEPTH=SF>=2), loaded once per band at address ``sf`` -- one write
    per memstream per band, so no back-to-back config writes. Per tile: MW=K_pad, PE=N/nt, NF=1."""
    t = tile if tile is not None else _geom.fold_plan(*shape, **plan_kwargs)["tile"]
    p = plan if plan is not None else _geom.fold_plan(*shape, **plan_kwargs)
    ...
    if NF != 1 or SF < 2:
        raise NotImplementedError(
            f"two-operand N-tiled shim requires NF=1 (PE=N_tile) and a temporal K fold "
            f"(SF>=2, memstream); got PE={PE} N_tile={ntile} NF={NF} SF={SF}. The fully-spatial "
            f"SF=1 N-tiling case (register weights) is a later step.")
    fb = 1 if force_behavioral else 0
    N, M = p["n"], p["num_input_vectors"]
    WW, ACCU = t["weight_width"], t["accu_width"]
    WB = t["weight_stream_width_ba"]
    ...
    def _w(n):
        return max(1, (n - 1).bit_length())
    # per-tile packed weight word: column block ti, from the buffered band (SIMD full-N rows)
    #   wword_ti[(pe*SIMD+s)*WW +: WW] = band[s][(ti*N_tile+pe)*WW +: WW]
    wwords = "\n".join(
        f"""    reg  [{WB - 1}:0]  wword_{ti};
    always @* begin
        wword_{ti} = {{{WB}{{1'b0}}}};
""" + "\n".join(
            f"        wword_{ti}[{(pe * SIMD + s) * WW} +: {WW}] = band[{s}][{(ti * ntile + pe) * WW} +: {WW}];"
            for pe in range(PE) for s in range(SIMD)) + "\n    end"
        for ti in range(nt))
    tiles = "\n".join(f"""    wire [{WB - 1}:0] w_odat_{ti};
    wire             w_ovld_{ti}, w_ordy_{ti}, wgt_tvalid_{ti}, wgt_tready_{ti};
    wire [{PB - 1}:0] out_tdata_{ti};
    memstream #(.DEPTH({DEPTH}), .WIDTH({WB}), .INIT_FILE(""), .RAM_STYLE("auto")) wmem_{ti} (
        .clk(ap_clk), .rst(ap_rst),
        .config_ce(cfg_ce), .config_we(cfg_we), .config_address(cfg_addr), .config_d0(wword_{ti}),
        .config_rack(), .config_q0(),
        .ordy(w_ordy_{ti}), .ovld(w_ovld_{ti}), .odat(w_odat_{ti})
    );
    assign wgt_tvalid_{ti} = ap_ce & run & w_ovld_{ti};
    assign w_ordy_{ti}     = ap_ce & run & wgt_tready_{ti};
    assign p_din[{(ti + 1) * PB - 1}:{ti * PB}] = out_tdata_{ti};   // N-block {ti} outputs
    mvu_vvu_axi #(
        .IS_MVU({t['is_mvu']}), .COMPUTE_CORE("{t['compute_core']}"),
        ...
        .ap_clk(ap_clk), .ap_clk2x(1'b0), .ap_rst_n(rst_n),
        .s_axis_weights_tdata(w_odat_{ti}), .s_axis_weights_tvalid(wgt_tvalid_{ti}), .s_axis_weights_tready(wgt_tready_{ti}),
        .s_axis_input_tdata(a_dout),
        .s_axis_input_tvalid(in_tvalid), .s_axis_input_tready(in_tready[{ti}]),
        .m_axis_output_tdata(out_tdata_{ti}), .m_axis_output_tvalid(out_tvalid[{ti}]), .m_axis_output_tready(out_tready)
    );""" for ti in range(nt))
    return f"""// Generated by gemm-ip-gen (mvau target). N-tiled two-operand (gemm_stream) shim:
// {nt} MVU cores, each the full-K reduction for its {ntile}-column block; activation broadcast,
// B column-sliced per tile, outputs concatenated. B loaded at runtime into per-tile memstreams.
// Tile: MW(K)={t['mw']} MH(N/tile)={ntile} PE={PE} SIMD={SIMD} SF={SF} N_TILES={nt} \
core={t['compute_core']} ACCU={ACCU} DEPTH={DEPTH} M={M}
module {module_name} (
    input  wire                 ap_clk,
    input  wire                 ap_rst,
    ...
    // activation FIFO (input)  {AB} = ceil(SIMD*ACTIVATION_WIDTH/8)*8 (broadcast to all tiles)
    input  wire [{AB - 1}:0] a_dout,
    input  wire                 a_empty_n,
    output wire                 a_read,
    // B FIFO (input)           {BB} = ceil(N*WEIGHT_WIDTH/8)*8 (N-wide K-row beats)
    ...
    input  wire                 b_empty_n,
    output wire                 b_read,
    // output FIFO (output)     {PB_TOTAL} = N_TILES * ceil(PE*ACCU_WIDTH/8)*8 (concatenated)
    output wire [{PB_TOTAL - 1}:0] p_din,
    input  wire                 p_full_n,
    output wire                 p_write
);
    ...
    reg  [1:0]            state = FILL;
    reg  [{_w(SIMD) - 1}:0]  sc = 0;    // lane within band (0..SIMD-1)
    reg  [{_w(SF) - 1}:0]  sfc = 0;     // band / word index (0..SF-1)
    reg  [1:0]            dcnt = 0;
    reg  [{_w(M) - 1}:0]  ocnt = 0;

    reg  [{BB - 1}:0]  band [0:{SIMD - 1}];   // SIMD full-N K-rows of the current band
    assign b_read = ap_ce & (state == FILL) & b_empty_n;
...
{wwords}
    ...
    wire [31:0] cfg_addr = sfc;
    wire        run = (state == RUN);

    wire in_tvalid  = ap_ce & run & a_empty_n;
    wire out_tready = ap_ce & p_full_n;
    wire [{nt - 1}:0] in_tready;
    wire [{nt - 1}:0] out_tvalid;
    assign a_read  = ap_ce & run & (&in_tready);
    assign p_write = ap_ce & (&out_tvalid);

    always @(posedge ap_clk) begin
        if (ap_rst) begin
            state <= FILL; sc <= 0; sfc <= 0; dcnt <= 0; ocnt <= 0;
        end else if (ap_ce) begin
            case (state)
                FILL: if (b_read) begin
                    band[sc] <= b_dout;
                    if (sc == {SIMD - 1}) begin sc <= 0; state <= WRITE; end
                    else sc <= sc + 1'b1;
                end
                WRITE: begin      // one write per memstream at addr sfc (per-tile column slice)
                    if (sfc == {SF - 1}) begin sfc <= 0; state <= DRAIN; dcnt <= 0; end
                    else begin sfc <= sfc + 1'b1; state <= FILL; end
                end
                DRAIN: begin
