...
def _2op_core_twin(p, t, func_name):
    """Two-operand C twin (single-tile fold, any DEPTH=NF*SF>=1 incl. fully-spatial
    DEPTH==1): both A and B are
...
def generate_2op_tb(shape, top_name="mvau_top", func_name="mvau_core", seed=42, plan=None,
                    n_nodes=6, **kw):
    """Public entry for the two-operand self-checking TB (single-tile only, see
    ``generate_2op_core_twin``).
