...
that the firmware includes, routing every ``nnet::gemm_stream_weightless<..,configN>``
...
generic target, with the weightless entry renamed) is the dispatch *primary* — so any
...
``<name>_gemm_stream_weightless`` entry.
...
``gemm_stream_weightless`` (an ODR clash) — the blackbox IP was never actually called.
"""
...
def unified_combined_header(layers):
    ...
    ``gemm_ip_dispatch<id>`` specialization forwarding to ``<name>_gemm_stream_weightless``.
    """
    # Soft blanket, weightless entry renamed so the single public entry is the dispatcher.
    soft = _ghls._GEMM_IP_COMBINED_FUNCS.replace(
        "gemm_stream_weightless", "gemm_soft_stream_weightless")
    ...
    return (
        ...
        "// Routes nnet::gemm_stream_weightless<..,CONFIG_T> to each layer's IP by\n"
        ...
        "    static void stream_weightless(hls::stream<data_T> &a, hls::stream<res_T> &r,\n"
        "                                  typename CONFIG_T::bias_t b[CONFIG_T::n_out]) {\n"
        "        gemm_soft_stream_weightless<data_T, res_T, CONFIG_T>(a, r, b);\n"
        "    }\n"
        "};\n"
        f"{specs}\n\n"
        "// The single io_stream weightless entry hls4ml calls; routes by config id.\n"
        "template <class data_T, class res_T, typename CONFIG_T>\n"
