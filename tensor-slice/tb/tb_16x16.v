...
module tb_16x16;
        ...
        for (r=0; r<2; r=r+1) begin : row
            for (c=0; c<2; c=c+1) begin : col
                ...
                tensor_slice_int8 slice (
                    ...
                    .final_mat_mul_size(8'd16), .a_loc(r[4:0]), .b_loc(c[4:0])
                );
            end
        end
    endgenerate
    ...
    initial begin
        ...
        #10 start_mat_mul = 0;
        fork
            begin repeat(15) @(posedge clk); a_data[0] <= 0; a_data[1] <= 0; b_data[0] <= 0; b_data[1] <= 0; end
            wait(done[1][1]);
        join
        $display("16x16 Latency: %0d cycles", ($time-30)/10);
        #20 $finish;
    end
endmodule
